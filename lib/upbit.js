...
UpbitAPI.prototype.upbitCoinSet = async function(connection){
  ...
  for(let i=0;i<upRes.length;i++){
    if(upRes[i].market.indexOf("KRW-")>-1){
      if(!dbMarkets.includes(upRes[i].market)){
        insertExe = true;
        insertQuery += "('"+upRes[i].market+"','"+upRes[i].korean_name+"'),";
      }
        
      upbitMarkets.push(upRes[i].market);
      
    }
  }
  
  for(let market of dbMarkets){
    if(!upbitMarkets.includes(market)){
      remove.push(market);
    }
  }
  ...
  if(remove.length>0){
    var updateQuery = "DELETE FROM upbit_coin WHERE market IN (";
    for(let rmMarket of remove){
      updateQuery += "'"+rmMarket+"',";
    }
    updateQuery = updateQuery.slice(0,-1)
    updateQuery += ")";
    var result = await connection.execute(updateQuery);
...
UpbitAPI.prototype.getTicker = async function(connection){
  ...
  const [data, fields] = await connection.execute("SELECT market FROM upbit_coin");
	var markets = []
...
UpbitAPI.prototype.recordingPrice = async function(connection){
  const priceData = await this.getTicker(connection);
  var insertQuery = "INSERT INTO price(date_key,slug,price) VALUES "
  var date = new Date();
  var currentMin = date.getMinutes();
  for(const data of priceData){
    const date_key = data.trade_date_kst + data.trade_time_kst.substr(0,2)+currentMin;
    insertQuery += "('"+date_key+"','"+data.market+"','"+data.trade_price+"'),";
  }
...
UpbitAPI.prototype.getPriceDataPeriod = async function(period=1, count=100){
  var date = new Date();
  date.setTime(date.getTime() - (1000*60*100))  //100분
  
  const dateId = dateFormat(date, "yyyymmddhhMM");
  console.log('date formet',dateId)
/*
  const [data, fields] = await connection.execute("SELECT market,weight FROM upbit_coin");
	var dbMarkets = []
  for(let i=data.length-1; i>=0; i--){
...
UpbitAPI.prototype.coinInfo = async function(minutes,market,count){
  return await this.request("/v1/candles/days/"+minutes,{market:market,count:count},"GET");
}
...
UpbitAPI.prototype.request = async function(apiUrl,body,type,qs={}){
  ...
  var options;
	...
	try{
		...
		if(type=="GET"){
      console.log('test', server_url + apiUrl+"?" + query)
			options = {
				method: "GET",
