...
UpbitAPI.prototype.upbitCoinSet = async function(connection){
  var updateQuery = "UPDATE upbit_coin SET tradeStatus = 1 "
  var insertExe = false;
  ...
  for(let i=0;i<upRes.length;i++){
    if(upRes[i].market.indexOf("KRW-")>-1){
      let dbIdx = dbMarkets.includes(upRes[i].market);
      if(dbIdx <0){
        insertExe = true;
        insertQuery += "('"+upRes[i].market+"','"+upRes[i].korean_name+"'),";
      }else{
        dbMarkets = dbMarkets.slice(dbIdx,1);
      }
  ...
  if(insertExe){
    ...
    console.log('insertExe',insertExe)
    //var result = await connection.execute(insertQuery);
  }

  if(dbMarkets.length>0){
    console.log('remove',dbMarkets)
  }
...
UpbitAPI.prototype.getTicker = async function(connection){
  ...
  markets = "KRW-ZRX,KRW-ZIL,KRW-XTZ,KRW-XRP,KRW-XLM,KRW-XEM,KRW-WAXP,KRW-WAVES,KRW-VET,KRW-UPP,KRW-TT,KRW-TSHP";
