      ...
      */
      ...
      if (result.state == "done") {
        ...
        if (status[0].status == 1) {
          ...
          await sell( trade[0].type, trade[0].lockAmount, coinPrice * 1.010, false, trade[0].slug, "upbit" );
      ...
      //구매 판매 확인 프로세스
        ...
        console.log(lastRSI15, coinInfo[0].trade_price, result.price *0.99)
...
async function upbitTrade(connection) {
  ...
  for (let x = 0; x < upData.length; x++) {
    ...
    if (valueStatus == 3) {
      ...
      await sell(type, lockAmount, lastPrice * 1.010, false, slug, "upbit");
    }
  }
}
